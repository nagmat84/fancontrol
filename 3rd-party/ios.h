...
class ios : public ios_base {
	...
	protected:
		ostream* _M_tie;
		mutable char _M_fill;
		mutable bool _M_fill_init;
		streambuf* _M_streambuf;

		const ctype<char>* _M_ctype;
	...
	public:
      ...
      explicit ios(streambuf* __sb) :
		 ...
		 */
		ostream* tie() const { return _M_tie; }
		 ...
		 */
		ostream* tie(ostream* __tiestr) {
			ostream* __old = _M_tie;
			_M_tie = __tiestr;
			return __old;
		}
		 ...
		 */
		streambuf* rdbuf() const { return _M_streambuf; }
		 ...
		 *  std::streambuf*  p = .....;
		 *
		 *  foo.ios::rdbuf(p);            // ios == ios
		 *  @endcode
		 */
		streambuf* rdbuf(streambuf* __sb);
		 ...
		 *
		 *  Maps a character of @c char to a character of @c char,
		 *  if possible.
		 *
		 *  Returns the result of
		 *  @code
		 *    std::use_facet<ctype<char> >(getloc()).narrow(c,dfault)
		 *  @endcode
		 *
		 *  Additional l10n notes are at
		 *  http://gcc.gnu.org/onlinedocs/libstdc++/manual/localization.html
		 ...
		 *  Returns the result of
		 *  @code
		 *    std::use_facet<ctype<char> >(getloc()).widen(c)
		 *  @endcode
		 *
		 *  Additional l10n notes are at
		 *  http://gcc.gnu.org/onlinedocs/libstdc++/manual/localization.html
    ...
    protected:
		...
		void move(ios&& __rhs) { this->move(__rhs); }

		void swap(ios& __rhs) noexcept {
		...
		void set_rdbuf(streambuf* __sb) { _M_streambuf = __sb; }
